...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    # On transient Supabase connection errors, reconnect for the next request
    err_str = str(exc)
    if any(x in err_str for x in ["ReadError", "ConnectError", "Resource temporarily unavailable"]):
        try:
            from db import _reconnect
            _reconnect()
        except Exception:
            pass
