...
def get_entries(
    ...
    shift_group_id: Optional[str] = None,   # filter by shift group (Morning/Afternoon/Night)
    user=Depends(get_current_user)
):
    # If filtering by shift GROUP, first resolve which shift IDs belong to it
    resolved_shift_ids: list = []
    if shift_group_id:
        sg_rows = supabase_admin.table("shifts").select("id").eq("shift_group_id", shift_group_id).execute().data
        resolved_shift_ids = [r["id"] for r in (sg_rows or [])]
        if not resolved_shift_ids:
            # No shifts in this group → no results possible
            return []
    ...
    if resolved_shift_ids:
        q = q.in_("shift_id", resolved_shift_ids)
    elif shift_id:
