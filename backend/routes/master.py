...
def db(query):
    """Execute a Supabase query and raise a clean 503 on DB errors instead of 500."""
    try:
        result = query.execute()
        return result
    except Exception as e:
        raise HTTPException(status_code=503, detail=f"Database error: {type(e).__name__} — {str(e)[:120]}")
...
class ProductBody(BaseModel):
    ...
    is_active: Optional[bool] = None
    persons_rate_json: Optional[str] = None  # JSON string of [{persons,rate}]
...
class ShiftGroupBody(BaseModel):
    name: Optional[str] = None
    start_time: Optional[str] = None
    end_time: Optional[str] = None
    is_active: Optional[bool] = None

@router.get("/shifts", response_class=JSONResponse)
def get_shifts(shift_group_id: Optional[str] = None, user=Depends(get_current_user)):
    q = supabase_admin.table("shifts").select("*, shift_groups(id,name)").order("start_time")
    if shift_group_id:
        q = q.eq("shift_group_id", shift_group_id)
    return q.execute().data

# ── Shift Groups ──
@router.get("/shift-groups")
def get_shift_groups(user=Depends(get_current_user)):
    return supabase_admin.table("shift_groups").select("*").order("start_time").execute().data

@router.post("/shift-groups")
def create_shift_group(body: ShiftGroupBody, user=Depends(require_admin)):
    ...
    data = {k: v for k, v in body.dict().items() if v is not None}
    res = supabase_admin.table("shift_groups").insert(data).execute()
    return res.data[0]

@router.put("/shift-groups/{group_id}")
def update_shift_group(group_id: str, body: ShiftGroupBody, user=Depends(require_admin)):
    data = {k: v for k, v in body.dict().items() if v is not None}
    res = supabase_admin.table("shift_groups").update(data).eq("id", group_id).execute()
    return res.data[0]

@router.delete("/shift-groups/{group_id}")
def delete_shift_group(group_id: str, user=Depends(require_admin)):
    supabase_admin.table("shift_groups").delete().eq("id", group_id).execute()
    return {"deleted": True}
...
def update_user(user_id: str, body: UserUpdateBody, user=Depends(require_admin)):
    if body.role and body.role not in ("admin", "product_lead", "operator", "viewer"):
