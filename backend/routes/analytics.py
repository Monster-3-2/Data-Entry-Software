...
def get_summary(
    ...
    # FIX: removed product_master(model_name) — product_master has no model_name column.
    # model name comes from models(name) via model_id on production_entries.
    ...
    # Aggregate per line (for KPI cards)
    lines_map = {}
    model_map  = {}
    for e in entries:
        lid   = e["line_id"]
        ...
        mname = (e.get("models") or {}).get("name", "—")   # from models table, not product_master

        if lid not in lines_map:
            lines_map[lid] = {"line_id": lid, "line_name": lname, "output": 0, "target": 0}
...
def get_monthly_summary(
    ...
    for v in summary.values():
        ...
        peak_mp = v["total_manpower"] / count
