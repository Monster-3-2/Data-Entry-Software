...
const API = {
  _ready: false,      // true once health check passes
  _readyPromise: null, // deduplicates concurrent readiness checks

  // ── Poll /api/health until the server actually responds with 200 ──
  // Strategy: no-cors ping just proves TCP works; we need a real 200 from FastAPI.
  // We poll /api/health (simple endpoint, no auth, returns CORS headers).
  // If server returns 503 with no CORS, fetch() throws TypeError — we catch and retry.
  async _ensureReady() {
    if (this._ready) return;
    if (this._readyPromise) return this._readyPromise;

    this._readyPromise = (async () => {
      const MAX_WAIT_MS  = 90000;  // 90s total (Render can take ~50s cold start)
      const POLL_EVERY   = 3000;   // poll every 3s
      const start        = Date.now();

      console.log('[API] Checking server readiness…');

      while (Date.now() - start < MAX_WAIT_MS) {
        try {
          const ctrl = new AbortController();
          const t    = setTimeout(() => ctrl.abort(), 8000);
          // Use the health endpoint — it returns proper CORS headers when app is running
          ...
          if (res.ok) {
            // Verify it's actually FastAPI responding (not a load-balancer stub)
            const body = await res.json().catch(() => null);
            if (body && body.ready === true) {
              // If server responded very quickly, wait 2s for middleware/DB to stabilize
              // (FastAPI starts before all connections are ready, causing 503 on first real request)
              const elapsed = Math.round((Date.now() - start) / 1000);
              if (elapsed < 2) await new Promise(r => setTimeout(r, 2000));
              console.log(`[API] Server ready (${elapsed}s warm-up)`);
              this._ready        = true;
              this._readyPromise = null;
              return;
            }
            console.warn('[API] Health returned 200 but bad body — still warming:', body);
          } else {
            console.warn('[API] Health check returned', res.status, '— retrying…');
          }
        } catch (e) {
          // TypeError = CORS blocked (server returning 503 with no CORS headers) OR network error
          // Either way, server not ready yet
          const elapsed = Math.round((Date.now() - start) / 1000);
          console.warn(`[API] Server not ready yet (${elapsed}s) — retrying in ${POLL_EVERY/1000}s…`);
        }
        await new Promise(r => setTimeout(r, POLL_EVERY));
      }

      this._readyPromise = null;
      throw new Error('Backend did not become ready in 90s. Please check your Render service is deployed.');
    })();
  ...
  async request(method, path, body = null) {
    // Ensure server is ready before any request
    ...
    const doFetch = (timeoutMs) => {
      const ctrl = new AbortController();
      const t    = setTimeout(() => ctrl.abort(), timeoutMs);
      return fetch(`${CONFIG.BACKEND_URL}${path}`, { ...opts, signal: ctrl.signal })
        .finally(() => clearTimeout(t));
    };

    let res;
    try {
      res = await doFetch(30000);
    } catch (e) {
      // Network error after we thought server was ready — mark not ready and fail
      this._ready = false;
    ...
    if (res.status === 503 || res.status === 502) {
      // Server went down between ready check and request (rare) — mark not ready
      this._ready = false;
      throw new Error('Server restarting — please wait a moment and refresh.');
    }
    if (!res.ok) {
  ...
  // Legacy alias used in pages for proactive wake
  _wakeServer() { return this._ensureReady(); },
};
...
const CACHE_RULES = [
  ...
  { prefix: '/api/analytics/',      ttl: 20000  },  // 20s — short enough for period switching
...
// ── Bust analytics cache on demand (call when period selector changes) ──
window.bustAnalyticsCache = function() {
  Object.keys(_cache).forEach(k => { if (k.startsWith('/api/analytics/')) delete _cache[k]; });
};

// ── Prefetch master data — only after server confirmed ready ──
(function prefetchMaster() {
  const paths = [
    '/api/master/lines', '/api/master/shifts',
    '/api/master/reasons', '/api/master/shift-groups',
  ];
  // _ensureReady() will poll until FastAPI is actually responding.
  // Only then prefetch — avoids parallel 503s on cold start.
  API._ensureReady()
    .then(() => paths.forEach(p => API.get(p).catch(() => {})))
    .catch(() => {}); // silently ignore if server never comes up
})();
