...
create table if not exists user_profiles (
  ...
  name text not null,
  email text not null,
  role text not null default 'operator' check (role in ('admin','product_lead','operator','viewer')),
  is_active boolean default true,
  created_at timestamptz default now()
);
...
create table if not exists downtime_entries (
  ...
  start_time time,            -- e.g. 09:30 (downtime start)
  end_time   time,            -- e.g. 10:00 (downtime end)
  duration_minutes integer not null default 0,  -- computed from start/end or entered directly
  remarks text,
  created_at timestamptz default now()
);

-- Migration: add start_time and end_time columns if upgrading from previous version
alter table downtime_entries add column if not exists start_time time;
alter table downtime_entries add column if not exists end_time time;

-- ============================================================
-- INDEXES
-- ============================================================

-- Unique constraint: one entry per line + model + shift + date.
-- Same line + different model + same shift = ALLOWED (two models can run same hour).
-- Different line + same model + same shift = ALLOWED.
-- Only when all four match is it a duplicate.
alter table production_entries
  drop constraint if exists uq_production_line_shift_date;
alter table production_entries
  drop constraint if exists uq_production_line_model_shift_date;
alter table production_entries
  add constraint uq_production_line_model_shift_date
  unique (date, line_id, model_id, shift_id);
...
create policy "operator_read_lines" on lines for select using (
  exists (select 1 from user_profiles where id = auth.uid() and role in ('product_lead','operator','viewer'))
);
...
create policy "operator_own_production" on production_entries for insert with check (
  exists (select 1 from user_profiles where id = auth.uid() and role in ('product_lead','operator'))
...
-- ============================================================
-- SHIFT GROUPS (new): a Shift contains multiple Hours
-- The existing 'shifts' table is now 'hours'
-- ============================================================
...
-- Link each hour to a shift group (nullable for backward compat)
alter table shifts add column if not exists shift_group_id uuid references shift_groups(id) on delete set null;

-- Policies for shift_groups
alter table shift_groups enable row level security;
create policy if not exists "admin_all_shift_groups" on shift_groups for all using (
  exists (select 1 from user_profiles where id = auth.uid() and role = 'admin')
);
create policy if not exists "read_shift_groups" on shift_groups for select using (
  exists (select 1 from user_profiles where id = auth.uid() and role in ('admin','product_lead','operator','viewer'))
);

insert into shift_groups (name, start_time, end_time) values
  ('Morning Shift', '06:00', '14:00'),
  ('Afternoon Shift', '14:00', '22:00'),
  ('Night Shift', '22:00', '06:00')
on conflict do nothing;

-- Persons→Rate lookup table for each product
-- JSON array: [{"persons": 10, "rate": 32}, {"persons": 9, "rate": 28}]
alter table product_master add column if not exists persons_rate_json jsonb default '[]'::jsonb;
